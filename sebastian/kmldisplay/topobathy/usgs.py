#!/usr/bin/python
# David Newell
# kmldisplay/topobathy/usgs.py
# Display KML features in Google Earth
# USGS Numerical Topography/Bathymetry
...
def KMLout(bbox=["-74.4","40.4","-73.5","40.9"],ge_key=""):
        ...
        table = 'elev_data'
        dataset = GeoUtils.constants.ElevSrc.USGS
        ...
        # KML Header
        kml_output = '<Folder id="USGSTopoBathy">\n'
        kml_output += '<styleUrl>%s/kml_styles.py#check-hide-children</styleUrl>\n' % (BASE_URL,)
        kml_output += '<name>USGS Coastal Numerical Topography-Bathymetry</name><open>0</open><visibility>1</visibility>\n'
        kml_output += '<description>Data derived from NGDC 3 Second Coastal Relief Model.</description>\n'
...
if __name__ == "__main__":
        ...
        try:
                ...
                #qv["BBOX"] = "0,0,0,0"
                # Los Angeles
                qv["BBOX"] = '-118.28,33.704,-118.23,33.733'
                qv["GE_KEY"] = ''
        ...
        try:
                ...
                # Los Angeles
                bbox = str("-118.28,33.704,-118.23,33.733").split(",")
