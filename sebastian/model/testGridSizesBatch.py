...
#or (len(sys.argv) == 4 and not (sys.argv[3] == '10report' or isinstance(sys.argv[3],int)))
    ...
    for portdata in [portdata_dike, portdata_berm] :
        for port in portdata:
            ...
            min_val_width = .1
            min_val = 9999999.9
            for test_grid_height in range(10, 50):
                for test_grid_width in range(10, 50):
                    try:
                        h = test_grid_height / 100.0
                        w = test_grid_width / 100.0
                        ...
                        if error == True:
                            ...
                            print "path_volume: %s path_length: %s  h: %s  w: %s  avg_elev: %s tall: %s short: %s " % (vol, ShortestPath.length(), h, w, avg_elev, tallest_section_depth, shortest_section_depth, )

                            if (min_val > vol) :
                                print "new min volume, old: %s h: %s w: %s new: %s h: %s w : %s" % (min_val,min_val_height,min_val_width,vol,h,w)
                                min_val = vol
                                min_val_height = h
                                min_val_width = w
        ...
        print "optimal grid for: %s h: %s w: %s vol: %s" % (current_structure, min_val_height, min_val_width, min_val, )
